...
elev  <- 1663 * 0.3048       # Elevation of lake surface, m above sea level. Converting from ft to m based on Water authority elevation values
...
timeStep  <- 10   # Time interval between DO measurements, minutes
sensorDepth <- 1.6  # Depth of DO sensor, m
outName <- "FCR2018_22"    # Text to use in labeling outputs, e.g. 'Acton2008'. Character.
dirData  <- "./Data/Model_Input/2018_22"    # Directory where data file are located, e.g. 'C:/GLEON/Acton'. Character.
dataIn  <- c("FCR_2018_2022_DO.csv", "FCR_2018_2022_sensorTemp.csv", "FCR_2018_20212_TempProfiles.csv",
             "FCR_2018_2022_PAR.csv", "FCR_2018_2022_WindSpeed_EXO.csv")     # Names of files that contain the data. This should be a character vector of
...
dataPAR <- read.csv("./Data/Model_Input/2018_22/FCR_2018_22_PAR_EXO.csv")
...
dataWind <- read.csv("./Data/Model_Input/2018_22/FCR_2018_22_WindSpeed_EXO.csv")
...
dataSensorTemp <- read.csv("./Data/Model_Input/2018_22/FCR_2018_22_sensorTemp.csv")
...
if(ModelVersion[5]==0|ModelVersion[5]==3|ModelVersion[5]==4){
  ...
  dataDO <- read.csv("./Data/Model_Input/2018_22/FCR_2018_22_DO.csv")
  dataDO$dateTime <- as.POSIXct(dataDO$dateTime)
  
  
  #Temp profile
  # dataTempProfile <- read.table(dataIn[3],header=T,sep='\t',colClasses=c(dateTime="POSIXct"))
  dataTempProfile <- read.csv("./Data/Model_Input/2018_22/FCR_2018_22_TempProfiles.csv")
...
sun$sunset<-sun$sunset+(60*60*hours.BeforeAfter) #change back and see what this should be. It is suppose to be from sunrise to sunrise
...
write.csv(fluxtest, "./zmix_fluxdummy_2018_22.csv")
