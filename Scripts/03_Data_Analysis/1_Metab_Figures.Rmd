...
metab_seasons_long %>% 
  ...
  mutate(median_sd = paste(round(median, 2), round(sd, 2), sep = " +- "),
         IQR =   paste(round(quantile25, 2), round(quantile75, 2), sep = " - ")  ) %>% 
  select(name, Season, IQR) %>% 
  pivot_wider(names_from = Season, values_from = IQR) %>% 
  select(name, Spring, Summer, Fall, Winter)
...
iceonoff_summaryStats_clean_IQR <- metab_seasons_long %>% 
  filter(!is.na(name),
         Season == "Winter") %>% 
  mutate(Ice_on_off = ifelse(icevector == 1, "Ice Cover", "No Ice Cover")) %>%  
  group_by(name, Ice_on_off) %>% 
  ...
  mutate(median_sd = paste(round(median, 2), round(sd, 2), sep = " +- "),
         IQR =   paste(round(quantile25, 2), round(quantile75, 2), sep = " - ")  ) %>% 
  select(name, Ice_on_off, IQR) %>% 
  pivot_wider(names_from = Ice_on_off, values_from = IQR)  
  
iceonoff_summaryStats_clean_IQR
...
metab_seasons_long_pdffig <- metab_seasons_long |> 
  mutate(name = ifelse(name == 'GPP_QAQC', "GPP", name),
         name = ifelse(name == 'R_QAQC', "R", name),
         name = ifelse(name == 'NEM_QAQC', "NEP", name))

pdf_all <- metab_seasons_long_pdffig |> 
  # mutate(Reservoir = "FCR") |> 
  ggplot()+
  geom_density(aes(value),fill = "pink",  alpha = 0.4)+ #fill = "lightgray"
...
pdf_seasons <- metab_seasons_long_pdffig |> 
  # filter(value < 9,
  #        value > -10) |> 
  ggplot()+
...
# pdf_seasons_nowinter <- metab_seasons_long_pdffig |> 
#   filter(Season != "Winter") |> 
#   ggplot()+
#   geom_density(aes(value, fill= as.factor(Season)), alpha = 0.2)+
...
# pdf_winter <- metab_seasons_long_pdffig |> 
#   filter(Season == "Winter") |> 
...
pdf_winter_gpp <- metab_seasons_long_pdffig |> 
  filter(Season == "Winter",
         name == "GPP") |> 
  mutate(Ice = ifelse(icevector == 1, "Ice", "No Ice")) |> 
  ggplot()+
...
pdf_winter_r <- metab_seasons_long_pdffig |> 
  filter(Season == "Winter",
         name == "R") |> 
  mutate(Ice = ifelse(icevector == 1, "Ice", "No Ice")) |> 
  ggplot()+
...
pdf_winter_nep <- metab_seasons_long_pdffig |> 
  filter(Season == "Winter",
         name == "NEP") |> 
  mutate(Ice = ifelse(icevector == 1, "Ice", "No Ice")) |> 
  ggplot()+
