...
Rseasonscomp_acrossyears_v2 <- 
  left_join(boxdata_grouped, dunnR_seasons_letters_list, by = c("Season" = "Group")) %>% 
  ggplot(aes(x = factor(Season, level = level_order), y = R))+   # , fill = Ice_on_off
  geom_boxplot(aes(fill = Season), position = position_dodge(preserve = "single"), alpha = 0.5, outlier.shape = NA)+ 
  geom_jitter(aes(shape = as.character(corrected_Year)), position=position_jitter(0.1), size = 3)+
  scale_shape_manual(values = c(11,21,22,23,24,25)) +
  stat_compare_means(method = "kruskal.test", label.y = -3.2, label.x = 1.3, size = 6, aes(label = sprintf("Kruskal-Wallis, p = %5.2f", as.numeric(..p.format..))))+ #round to 2 digits
  geom_text(aes(x = Season, label = Letter), y = 0, size = 6)+
  ylim(-3.2, 0)+
  labs(fill = element_blank(),
       shape = "Year",
       x = element_blank(),
...
GPPseasonscomp_acrossyears_v2 <- GPPseasonscomp_acrossyears + theme(legend.position = "top")

NEPseasonscomp_acrossyears_v2 <- NEPseasonscomp_acrossyears + theme(legend.position = "none")

fig5idea <- GPPseasonscomp_acrossyears_v2 / Rseasonscomp_acrossyears_v2 / NEPseasonscomp_acrossyears_v2

ggsave(filename = "../../Figures/Fig5idea.jpg",
       fig5idea, device = "jpg", width = 200, height = 350, units = "mm")
...
```{r}
 ...
 fig4top_v2 <- wintercomp_byyear_NEP + wintercomp_acrossyears + plot_layout(widths = c(7,3)) + plot_annotation(tag_levels = 'a')
...
ggsave(filename = "../../Figures/Fig4_justTop.jpg",
       fig4top_v2, device = "jpg", width = 350, height = 150, units = "mm")


fig5 <- Rseasonscomp_acrossyears + GPPseasonscomp_acrossyears + NEPseasonscomp_acrossyears + plot_annotation(tag_levels = 'a')
