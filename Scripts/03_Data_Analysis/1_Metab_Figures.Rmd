...
pdf_all <- metab_seasons_long |> 
  ggplot()+
  geom_density(aes(value), fill = "lightgray", alpha = 0.4)+
  facet_wrap(~name, scales = "free_x")+
  theme_bw() + theme(text = element_text(size=14), panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  labs( x = metab_label2)
...
pdf_seasons <- metab_seasons_long |> 
  ...
  ggplot()+
  geom_density(aes(value, fill= as.factor(Season)), alpha = 0.4)+
  facet_wrap(~name, scales = "free_x")+
  ...
  labs(fill = "Season", x = metab_label2)
...
#z <- ggpubr::ggarrange(pdf_all , pdf_seasons , pdf_winter, labels = "a", "b", "c", ncol = 3)

z <- cowplot::plot_grid(pdf_all, pdf_seasons, pdf_winter, labels = c("a", "b", "c"), ncol = 1)
...
more edits for pdf distibutions 
...
########## 7 panels 
all <- metab_seasons_long |> 
  ggplot()+
  geom_density(aes(value), fill = "black", alpha = 1)+
  facet_wrap(~name, scales = "free_x")+
  theme_bw() + theme(text = element_text(size=14), panel.grid.major = element_blank(), panel.grid.minor = element_blank())+  labs( x = element_blank())

spring <- metab_seasons_long |>
  filter(Season == "Spring") |> 
  ggplot()+
  geom_density(aes(value), fill = "darkturquoise", alpha = 1)+
  facet_wrap(~name, scales = "free_x")+
  theme_bw() + theme(text = element_text(size=14), panel.grid.major = element_blank(), panel.grid.minor = element_blank())+  labs( x = element_blank())

summer <- metab_seasons_long |>
  filter(Season == "Summer") |> 
  ggplot()+
  geom_density(aes(value), fill = "green4", alpha = 1)+
  facet_wrap(~name, scales = "free_x")+
  theme_bw() + theme(text = element_text(size=14), panel.grid.major = element_blank(), panel.grid.minor = element_blank())+  labs( x = element_blank())

fall <- metab_seasons_long |>
  filter(Season == "Fall") |> 
  ggplot()+
  geom_density(aes(value), fill = "darkorange3", alpha = 1)+
  facet_wrap(~name, scales = "free_x")+
  theme_bw() + theme(text = element_text(size=14), panel.grid.major = element_blank(), panel.grid.minor = element_blank())+  labs( x = element_blank())

winter <- metab_seasons_long |>
  filter(Season == "Winter") |> 
  ggplot()+
  geom_density(aes(value), fill = "slategray", alpha = 1)+
  facet_wrap(~name, scales = "free_x")+
  theme_bw() + theme(text = element_text(size=14), panel.grid.major = element_blank(), panel.grid.minor = element_blank())+  labs( x = element_blank())

ice_free <- metab_seasons_long |>
  filter(Season == "Winter",
         icevector == 0) |> 
  ggplot()+
  geom_density(aes(value), fill = "skyblue2", alpha = 1)+
  facet_wrap(~name, scales = "free_x")+
  theme_bw() + theme(text = element_text(size=14), panel.grid.major = element_blank(), panel.grid.minor = element_blank())+  labs( x = element_blank())

ice_covered <- metab_seasons_long |>
  filter(Season == "Winter",
         icevector == 1) |> 
  ggplot()+
  geom_density(aes(value), fill = "#FAF9F6", alpha = 1)+
  facet_wrap(~name, scales = "free_x")+
  theme_bw() + theme(text = element_text(size=14), panel.grid.major = element_blank(), panel.grid.minor = element_blank())+  labs( x = metab_label2)

zz <- cowplot::plot_grid(all, spring, summer, fall, winter, ice_free, ice_covered, 
                         labels = c("a", "b", "c", "d", "e", "f", "g"), ncol = 1, rel_heights = c(1,1,1,1,1,1,1.2))

zz

ggsave(filename = "../../Figures/newfig_pdf_long.jpg",
       zz, device = "jpg", width = 300, height = 300, units = "mm")
