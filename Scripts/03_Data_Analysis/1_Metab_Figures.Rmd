---
title: "Metabolism figures from using QAQCd metab model outputs"
...
## Testing affect of winter on annual metab on seasons classficiations
for both operational seasons, operation winter and physical summer, phy summer and winter seasons, and solar seasons 

```{r}
### current seasons fig

fig2_all_seasons_data_opWint_physSum <- metab_seasons_winterOper_summerPhys %>% 
...
fig2_nowinter_data_opWint_physSum <- metab_seasons_winterOper_summerPhys %>% 
...
fig2_data_opWint_physSum <- left_join(fig2_all_seasons_data_opWint_physSum, fig2_nowinter_data_opWint_physSum, by = "corrected_Year")


 x <- fig2_data_opWint_physSum %>% 
  ...
  ylim(-0.42, 0.1)+
   stat_compare_means(method = "t.test", label.y = 0.08, label.x = 1.3, size = 6)+ #t test 
  theme_bw()+
  ...
  ggtitle("operational winter \n physical summer")



### operational winter and summer season fig
fig2_all_seasons_data_operational <- metab_seasons_operational %>% 
  group_by(cor_Year) %>% 
  dplyr::summarise(annual_GPP_Allseasons = median(GPP_QAQC, na.rm = T),
                   annual_R_Allseasons = median(R_QAQC, na.rm = T),
                   annual_NEP_Allseasons = median(NEM_QAQC, na.rm = T)
...
fig2_nowinter_data_operational <- metab_seasons_operational %>% 
  filter(Season != "Winter") %>% 
  group_by(cor_Year) %>% 
  dplyr::summarise(annual_GPP_nowinter = median(GPP_QAQC, na.rm = T),
                   annual_R_nowinter = median(R_QAQC, na.rm = T),
...
fig2_data_operational <- left_join(fig2_all_seasons_data_operational, fig2_nowinter_data_operational, by = "cor_Year") %>% rename("corrected_Year" = "cor_Year")

w<- fig2_data_operational %>% 
  filter(corrected_Year %in% c(2016,2018,2019,2021)) %>% 
  select(corrected_Year, annual_NEP_nowinter, annual_NEP_Allseasons) %>% 
  pivot_longer(-1) %>% 
  mutate(name = ifelse(name == "annual_NEP_Allseasons", "All Seasons", "No Winter")) %>% 
...
fig2_all_seasons_data_phys <- metab_seasons_phys %>% 
  group_by(cor_Year) %>% 
  dplyr::summarise(annual_GPP_Allseasons = median(GPP_QAQC, na.rm = T),
                   annual_R_Allseasons = median(R_QAQC, na.rm = T),
                   annual_NEP_Allseasons = median(NEM_QAQC, na.rm = T)
...
fig2_nowinter_data_phys <- metab_seasons_phys %>% 
  filter(Season != "Winter") %>% 
  group_by(cor_Year) %>% 
  dplyr::summarise(annual_GPP_nowinter = median(GPP_QAQC, na.rm = T),
                   annual_R_nowinter = median(R_QAQC, na.rm = T),
...
fig2_data_phys <- left_join(fig2_all_seasons_data_phys, fig2_nowinter_data_phys, by = "cor_Year")
...
y<- fig2_data_phys %>% 
  ...
  theme(legend.position = "none")+
   theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(color="black"), axis.title.x = element_text(size=16), axis.title.y = element_text(size=16),
        legend.title = element_text(size=16),  plot.title = element_text(size=20),  axis.text = element_text(size=16,color="black"),  legend.text = element_text(size=16,color="black"))+
  ggtitle("physical winter \n physical summer")

 
### solar winter season fig
fig2_all_seasons_data_solar <- metab_seasons_solar %>% 
  group_by(cor_Year) %>% 
  dplyr::summarise(annual_GPP_Allseasons = median(GPP_QAQC, na.rm = T),
                   annual_R_Allseasons = median(R_QAQC, na.rm = T),
                   annual_NEP_Allseasons = median(NEM_QAQC, na.rm = T)
...
fig2_nowinter_data_solar <- metab_seasons_solar %>% 
  filter(Season != "Winter") %>% 
  group_by(cor_Year) %>% 
  dplyr::summarise(annual_GPP_nowinter = median(GPP_QAQC, na.rm = T),
                   annual_R_nowinter = median(R_QAQC, na.rm = T),
...
fig2_data_solar <- left_join(fig2_all_seasons_data_solar, fig2_nowinter_data_solar, by = "cor_Year")
...
# ggsave(filename = "../../Figures/Fig_S4_SeasonsTestsFig2.png",
#        Fig_S4, device = "png", width = 400, height = 250, units = "mm")

```



## Make figure 2b using both operational season
...
fig2_justwinter_data_operational <- metab_seasons_operational %>% 
  filter(Season == "Winter") %>% 
  group_by(cor_Year) %>% 
...
fig2_data_operational <- left_join(fig2_all_seasons_data_operational, fig2_nowinter_data_operational, by = "cor_Year") 
fig2_data_operational <- left_join(fig2_data_operational, fig2_justwinter_data_operational, by = "cor_Year") %>% 
  rename("corrected_Year" = "cor_Year")


#run t test 
fig2_data_ttest <- fig2_data_operational %>% 
    filter(corrected_Year %in% c(2016,2018,2019,2020,2021)) 

t.test(fig2_data_ttest$annual_NEP_nowinter, fig2_data_ttest$annual_NEP_Allseasons, paired = T)
# t.test(fig2_data_ttest$annual_R_nowinter, fig2_data_ttest$annual_R_Allseasons, paired = F)
# t.test(fig2_data_ttest$annual_GPP_nowinter, fig2_data_ttest$annual_GPP_Allseasons, paired = F)

#NEP plot 
# fig2_winterVSnowinter <- fig2_data_operational %>% 
#   filter(corrected_Year %in% c(2016,2018,2019,2020,2021)) %>% 
#   select(corrected_Year, annual_NEP_nowinter, annual_NEP_Allseasons) %>% 
#   pivot_longer(-1) %>% 
#   mutate(name = ifelse(name == "annual_NEP_Allseasons", "All Seasons", "No Winter")) %>% 
...
#   #stat_compare_means(method = "t.test", paired = T, label.y = 0.1, label.x = 1.3)+ #t test 
#   # geom_point(aes(x = name, y = value), shape=1, size=6, color="black")+
#   labs(x = element_blank(), y = nep_label,
#        color = "Year")+
#   geom_hline(yintercept = 0, linetype=2, size = 0.75) +
#   ylim(-0.4, 0.1)+
...
# 
# fig2_winterVSnowinter


### adding just winter to the figure 
 level_fig2 <- c("All Seasons", "No Winter", "Just Winter")
 
fig2_winterVSnowinter_vsjustwinter <- fig2_data_operational %>% 
  filter(corrected_Year %in% c(2016,2018,2019,2020,2021)) %>% 
  ...
  pivot_longer(-1) %>% 
    mutate(name = ifelse(name == "annual_NEP_Allseasons", "All Seasons", name),
           ...
           name = ifelse(name == "annual_NEP_justwinter", "Just Winter", name)) %>% 
  ggplot(mapping = aes(x= factor(name, level = level_fig2), y = value))+
  geom_boxplot()+
  geom_jitter(aes(color = as.factor(corrected_Year)), size = 4, position=position_jitter(0))+
    stat_compare_means(method = "kruskal.test", label.y = -0.32, label.x = 1.3, size = 6)+
  labs(x = element_blank(), y = nep_label,
       color = element_blank())+
...
fig2_winterVSnowinter_vsjustwinter
...
full_ts <- ggplot(data = metab_seasons, aes(x = Date, y = GPP_QAQC))+
  geom_point(shape = 21, color="black", fill ="seagreen3",size=4, alpha = 0.5)+ # green 3,  lightgreen, seagreen3, springgreen3
  geom_hline(yintercept = 0, linetype=2, size = 1.5) +
  geom_point(aes(x=Date,y=(R_QAQC*-1)),shape = 21, color = "black", fill ="brown",size=4, alpha = 0.5)+ #gray30 for darker outline
  #geom_line(aes(x = Date, y = NEP_7day_rollmean), size = 1, color = "black", alpha = 0.8)+ #7 day rolling mean
  #geom_smooth(aes(x= Date, y = NEM_QAQC))+
  scale_x_date(breaks = seq(ymd("2016-01-01"), ymd("2022-03-01"), by="12 months"),date_labels="%Y")+ #date_labels="%b %Y"
  #scale_y_continuous(limits=c(-15,10), breaks=seq(-30,20, by=5))+
...
# ggsave(filename = "../../Figures/MS_drafts/GPP_R_full_timeseries_NEP7day.png",       
#        full_ts, device = "png", width = 230, height = 150, units = "mm")
...
# ggsave(filename = "./Figures/GLEON2022/NEP_full_TS_rollmean.tiff", 
#        NEP_full_ts, device = "tiff", width = 230, height = 150, units = "mm")
...
# fig2 <- full_ts + fig2_winterVSnowinter + 
#   plot_layout(widths = unit(c(5,2),c("null","null"))) +  plot_annotation(tag_levels = 'a') & theme(plot.tag = element_text(size = 24))
# fig2
# 
# # ggsave(filename = "../../Figures/MS_drafts/Fig2_median.png",
# #        fig2, device = "png", width = 350, height = 200, units = "mm")

GPP_R_overNEP <- (full_ts / NEP_full_ts) #+ plot_annotation(tag_levels = 'a') & theme(plot.tag = element_text(size = 24))

fig2_withNEPpannel <- cowplot::plot_grid(GPP_R_overNEP , fig2_winterVSnowinter_vsjustwinter, rel_widths = c(6,4))

# ggsave(filename = "../../Figures/Fig2.png",
#        fig2_withNEPpannel, device = "png", width = 350, height = 200, units = "mm")
...
metab_seasons <- metab_seasons %>% rename(corrected_Year = cor_Year) %>% 
...
seasons_summaryStats_clean <- metab_seasons_long %>% 
  ...
  mutate(median_sd = paste(round(median, 2), round(sd, 2), sep = " +- "),
         range =   paste(round(min, 2), round(max, 2), sep = " - ")  ) %>% 
  select(name, Season, median_sd) %>% 
  pivot_wider(names_from = Season, values_from = median_sd) 
...
iceonoff_summaryStats_clean <- metab_seasons_long %>% 
  ...
  mutate(median_sd = paste(round(median, 2), round(sd, 2), sep = " +- "),
         range =   paste(round(min, 2), round(max, 2), sep = " - ")  ) %>% 
  select(name, Ice_on_off, median_sd) %>% 
  pivot_wider(names_from = Ice_on_off, values_from = median_sd)  
...
# ggsave(filename = "../../Figures/SupFig_S5_metab_acrossyears.png",
#        SupFig_S5, device = "png", width = 350, height = 200, units = "mm")
...
#make figure that just groups all data into seasons for sup fig X
# gpp_acrossseasons_kruskal <- metab_seasons %>% 
#   ggplot(aes(x = as.factor(Season), y = GPP_QAQC))+ 
#   geom_boxplot(alpha = 0.5)+
#   geom_jitter(position=position_jitter(0.1))+
#   labs(x = element_blank(),  y = gpp_label)+
#   ylim(0,17)+
#   stat_compare_means(method = "kruskal.test", 
#                      label.y = 15, label.x = 1.3, size = 6 )+ # , aes(label = paste0("p = ", after_stat(p.format)))
#   theme_bw(base_size = 14)+
#   theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(color="black"), axis.title.x = element_text(size=16), axis.title.y = element_text(size=16),
#         legend.title = element_text(size=16),  plot.title = element_text(size=20),  axis.text = element_text(size=16,color="black"),  legend.text = element_text(size=16,color="black"))
# 
# r_acrossseasons_kruskal <- metab_seasons %>% 
#   ggplot(aes(x = as.factor(Season), y = R_QAQC))+ 
#   geom_boxplot(alpha = 0.5)+
#   geom_jitter(position=position_jitter(0.1))+
#   labs(x = element_blank(),  y = r_label)+
#   ylim(0,17)+
...
#         legend.title = element_text(size=16),  plot.title = element_text(size=20),  axis.text = element_text(size=16,color="black"),  legend.text = element_text(size=16,color="black"))
# 
# nep_acrossseasons_kruskal <- metab_seasons %>% 
#   ggplot(aes(x = as.factor(Season), y = NEM_QAQC))+ 
#   geom_boxplot(alpha = 0.5)+
#   geom_jitter(position=position_jitter(0.1))+
#   labs(x = element_blank(),  y = nep_label)+
#   stat_compare_means(method = "kruskal.test", 
#                      label.y = -10, label.x = 1.3, size = 6 )+ # , aes(label = paste0("p = ", after_stat(p.format)))
#   theme_bw(base_size = 14)+
#   theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(color="black"), axis.title.x = element_text(size=16), axis.title.y = element_text(size=16),
#         legend.title = element_text(size=16),  plot.title = element_text(size=20),  axis.text = element_text(size=16,color="black"),  legend.text = element_text(size=16,color="black"))
# 
# SupFig_SX_acrossseasons <- gpp_acrossseasons_kruskal / r_acrossseasons_kruskal / nep_acrossseasons_kruskal
# 
# ggsave(filename = "../../Figures/SupFig_SX_metab_acrossseasons.png",
#        SupFig_SX_acrossseasons, device = "png", width = 350, height = 200, units = "mm")
...
metab2022 <- metab_seasons %>% 
  ...
  geom_rect(data= rect1, aes(xmin=as.Date("2022-01-11"), xmax=as.Date("2022-01-13"), ymin= -Inf, ymax=Inf), fill="blue", alpha=0.2, inherit.aes = FALSE) +
  geom_rect(data= rect2, aes(xmin= as.Date("2022-01-16"), xmax= as.Date("2022-02-09"), ymin= -Inf, ymax=Inf), fill="blue", alpha=0.2, inherit.aes = FALSE) +
  # labs(y=metab_label, x=NULL)+ 
  labs(y=NULL, x=NULL)+
  geom_hline(yintercept = 0, linetype=2, size = 1.5) +
  geom_point(shape = 21, color="black", fill ="seagreen3",size=4)+ # green 3,  lightgreen, seagreen3, springgreen3
  geom_point(aes(x=Date,y=(R_QAQC*-1)),shape = 21, color = "black", fill ="brown",size=4)+ #gray30 for darker outline
  #geom_point(aes(x=Date,y=NEM_QAQC),shape = 21, color = "black", fill ="pink",size=4)+
  #geom_smooth(aes(x=Date, y=NEM_QAQC))+
...
metab2021 <- metab_seasons %>% 
  filter(Date <= ymd("2021-02-22"),
         Date >= ymd("2020-12-20")) %>% 
  ggplot()+
  # scale_y_continuous(limits=c(-5.5,3), breaks=seq(-7,5, by=1))+
  ylim(-3,2)+
  ...
  geom_rect(data= rect1, aes(xmin=as.Date("2020-12-27"), xmax=as.Date("2020-12-29"), ymin=-Inf, ymax=Inf), fill="blue", alpha=0.2, inherit.aes = FALSE) +
  geom_rect(data= rect2, aes(xmin= as.Date("2021-01-10 00:00:00"), xmax= as.Date("2021-02-08 00:00:00"), ymin=-Inf, ymax=Inf), fill="blue", alpha=0.2, inherit.aes = FALSE) +
  geom_rect(data= rect3, aes(xmin= as.Date("2021-02-11 00:00:00"), xmax= as.Date("2021-02-22 00:00:00"), ymin=-Inf, ymax=Inf), fill="blue", alpha=0.2, inherit.aes = FALSE) +
  # labs(y=metab_label, x=NULL)+ 
  labs(y=NULL, x=NULL)+   
  geom_hline(yintercept = 0, linetype=2, size = 1.5) +
...
metab2020 <- metab_seasons %>% 
  filter(Date <= ymd("2020-02-22"),
         Date >= ymd("2019-12-20")) %>% 
  ggplot(aes(x = Date, y = GPP_QAQC))+
  # scale_y_continuous(limits=c(-7,2), breaks=seq(-7,5, by=1))+
  ylim(-3,2)+
  #labs(y=metab_label, x=NULL)+
  labs(y=NULL, x=NULL)+
  geom_hline(yintercept = 0, linetype=2, size = 1.5) +
...
metab2019 <- metab_seasons %>% 
  ...
  geom_rect(data= rect1, aes(xmin=as.Date("2019-01-21"), xmax=as.Date("2019-01-24"), ymin=-Inf, ymax=Inf), fill="blue", alpha=0.2, inherit.aes = FALSE) +
  geom_rect(data= rect2, aes(xmin=as.Date("2019-01-27"), xmax=as.Date("2019-02-02"), ymin=-Inf, ymax=Inf), fill="blue", alpha=0.2, inherit.aes = FALSE) +
  geom_hline(yintercept = 0, linetype=2, size = 1.5) +
  geom_point(shape = 21, color="black", fill ="seagreen3",size=4)+ 
  geom_point(aes(x=Date,y=(R_QAQC*-1)),shape = 21, color = "black", fill ="brown",size=4)+
  #geom_point(aes(x=Date,y=NEM_QAQC),shape = 21, color = "black", fill ="pink",size=4)+
...
metab2018 <- metab_seasons %>% 
  ...
  geom_rect(data= rect1, aes(xmin=as.Date("2017-12-28"), xmax=as.Date("2018-01-08"), ymin=-Inf, ymax=Inf), fill="blue", alpha=0.2, inherit.aes = FALSE)+
  labs(y=NULL, x=NULL)+   
  geom_hline(yintercept = 0, linetype=2, size = 1.5) +
  geom_point(shape = 21, color="black", fill ="seagreen3",size=4)+ 
  geom_point(aes(x=Date,y=(R_QAQC*-1)),shape = 21, color = "black", fill ="brown",size=4)+
  #geom_point(aes(x=Date,y=NEM_QAQC),shape = 22, color = "black", fill ="pink",size=4)+
  #geom_smooth(aes(x=Date, y=NEM_QAQC))+
  geom_line(aes(x = Date, y = NEP_7day_rollmean), size = 2, color = "black", alpha = 0.8)+ #7 day rolling mean
...
#2016; skipping 17 since missing driver met data
metab2016 <- metab_seasons %>% 
  ...
  geom_rect(data= rect1, aes(xmin=as.Date("2016-01-21"), xmax=as.Date("2016-02-21"), ymin=-Inf, ymax=Inf), fill="blue", alpha=0.2, inherit.aes = FALSE)+
  geom_hline(yintercept = 0, linetype=2, size = 1.5) +
  geom_point(shape = 21, color="black", fill ="seagreen3",size=4)+ 
  geom_point(aes(x=Date,y=(R_QAQC*-1)),shape = 21, color = "black", fill ="brown",size=4)+
  #geom_point(aes(x=Date,y=NEM_QAQC),shape = 21, color = "black", fill ="pink",size=4)+
...
##run these two lines in the console
(metab2016 + metab2018 + metab2019) / (metab2020 + metab2021 + metab2022)
 grid::grid.draw(grid::textGrob(metab_label2, x = 0.01, rot = 90, gp = grid::gpar(fontsize = 11)))
#then have to manually export from plots window as png w/ 
# ggsave("../../Figures/MS_final/WinterTS_fig3.png", width = 800, height = 500, units = "mm")
...
icetotaldays <- data.frame(Year = c(2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022), #removing 2014 that had 35 days
                           Days_iceCover = c(49, 32, 3, 12, 9, 0, 35, 28))
icetotaldays$Year <- as.character(icetotaldays$Year)

plot(icetotaldays$Year, icetotaldays$Days_iceCover)

##ice dotted lines plot 
#2014
...
binded_icedata <- binded_icedata %>% 
  ...
  mutate(corrected_years = ifelse(Year == 2013 & Month == 12, 2014, Year),
         corrected_years = ifelse(Year == 2014 & Month == 12, 2015, corrected_years),
         corrected_years = ifelse(Year == 2015 & Month == 12, 2016, corrected_years),
...
binded_icedata_remove_zeros <- binded_icedata %>% 
  ...
  mutate(icevector_yn = ifelse(icevector == 1, "On", "Off"))  #filter(icevector == 1)
...
icecover_dotplots_B <- binded_icedata_remove_zeros %>% 
  ...
  filter(corrected_years >= 2016) %>%
  filter(fakeDate >= ymd("2023-12-20"),
         fakeDate <= ymd("2024-02-22")) %>% 
  ggplot(mapping = aes(x = fakeDate, y = corrected_years))+
  geom_tile(aes(fill = as.character(icevector_yn_A)))+ #, alpha = 0.2
  # scale_x_continuous(limits = c(0,125), breaks = c(0, 30, 60, 90, 120))+
  scale_x_date(breaks = seq(min(ymd("2023-12-20")), max(ymd("2024-02-22")), by = "21 days") , date_labels = "%d %b")+
  ...
  theme(legend.position="top",
        legend.background =  element_rect(color = "black", linetype = "solid", fill = "lightgray"))+
  ...
  #scale_fill_manual(values=c("#30D5C8", "#D3D3D3"))
   scale_fill_manual(values=c("#FAF9F6", "lightblue"))


icecover_dotplots_B

ggsave(filename = "../../Figures/Fig1b_icecover.png",
       icecover_dotplots_B, device = "png", width = 10.6, height = 20, units = "in")
...
wintercomp_byyear_NEP <- winterNEP %>% 
  ...
  #                    label.y = -6, label.x = 1, size = 6 )+
   stat_compare_means(method = "wilcox.test", label.y = -6, label.x = 1, size = 6, aes(label = sprintf("p = %5.2f", as.numeric(..p.format..))))+ #round to 2 digits
...
# ggsave(filename = "../../Figures/FigS6_WinterComp_byyears.png",
#         wintercomps_byyears, device = "png", width = 350, height = 200, units = "mm")

### Winter NEP average to year 
wintercomp_acrossyears <- winterNEP %>% 
  ...
  geom_boxplot(position = position_dodge(preserve = "single"), alpha = 0.5, outlier.alpha = 0.5)+   #, outlier.alpha = 0
  ...
  # stat_summary(fun = mean, geom="point", shape=22, size=5, color="black", fill="black") + #point mean
  geom_hline(yintercept = 0, linetype=2, size = 0.75) +
  #stat_compare_means(method = "wilcox.test", label.y = 0.4, label.x = 1, size = 6,  aes(label = ..p.format..))+
  stat_compare_means(method = "wilcox.test", label.y = 0.4, label.x = 1, size = 6, aes(label = sprintf("Wilcoxon, p = %5.2f", as.numeric(..p.format..))))+ #round to 2 digits
  ylim(-1.5, 0.5)+
  labs(fill = element_blank(),
       shape = "Year",
       x = element_blank(),
...
b <- winterNEP %>% 
  ...
  summarise(NEP = mean(NEM_QAQC, na.rm = T),
            GPP = mean(GPP_QAQC, na.rm = T),
             R = mean(R_QAQC, na.rm = T)) #%>% 
  # filter(corrected_Year %in% c(2016,2019,2021,2022))

wilcox.test(NEP ~ icevector, data = b) 
...
#### make plots per year for supp figure 

seasonscomp_byyear_NEP <- boxdata %>% 
  filter(!is.na(NEM_QAQC)) %>% #removing NAs from data frame for plot to see what gets cut off by y limits 
  # filter(Season %in% c("Winter", "Summer")) %>% #If you want plot comparing just winter and summer
  ggplot(mapping = aes(y = NEM_QAQC, x = factor(Season, level = level_order)))+  # , fill = Season
  #geom_violin()+
  geom_boxplot(aes(fill = Season), position = position_dodge(0.9), alpha = 0.9)+
 geom_jitter(position=position_jitter(0.1), alpha = 0.3)+
  facet_wrap(~corrected_Year, nrow = 1)+
  # ylim(-3,3)+
  #scale_y_continuous(limits=c(-9, 4), breaks=seq(-15,5, by=3))+
  geom_hline(yintercept = 0, linetype=2, size = 0.75) +
 # stat_summary(fun=mean, geom="point", shape=4, size=3, color="black", position = position_dodge2(width = 0.9, preserve = "single"))+ #add the mean 
  stat_compare_means(method = "kruskal.test", aes(label = paste0("p = ", after_stat(p.format))), 
                     label.y = -10.1, label.x = 2, size = 6 ) +  # , aes(label = ..p.signif..)
  labs(x = NULL, y = nep_label)+
  theme_bw(base_size = 18)+
...
GPPseasonscomp_byyear <- boxdata %>% 
  filter(!is.na(NEM_QAQC)) %>% #removing NAs from data frame for plot to see what gets cut off by y limits 
  ggplot(mapping = aes(y = GPP_QAQC, x = factor(Season, level = level_order)))+  # , fill = Season
  geom_boxplot(aes(fill = Season), position = position_dodge(0.9), alpha = 0.9)+
 geom_jitter(position=position_jitter(0.1), alpha = 0.3)+
  facet_wrap(~corrected_Year, nrow = 1)+
  stat_compare_means(method = "kruskal.test", aes(label = paste0("p = ", after_stat(p.format))), 
                     label.y = 8, label.x = 2, size = 6 ) +  # , aes(label = ..p.signif..)
  labs(x = NULL, y = gpp_label)+
  theme_bw(base_size = 18)+
    scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  theme(legend.position = "none",
        axis.title.y = element_text(size=14),
...
Rseasonscomp_byyear <- boxdata %>% 
  filter(!is.na(NEM_QAQC)) %>% #removing NAs from data frame for plot to see what gets cut off by y limits 
  ggplot(mapping = aes(y = R_QAQC, x = factor(Season, level = level_order)))+  # , fill = Season
  geom_boxplot(aes(fill = Season), position = position_dodge(0.9), alpha = 0.9)+
 geom_jitter(position=position_jitter(0.1), alpha = 0.3)+
  facet_wrap(~corrected_Year, nrow = 1)+
  stat_compare_means(method = "kruskal.test", aes(label = paste0("p = ", after_stat(p.format))), 
                     label.y = 12, label.x = 2, size = 6 ) +  # , aes(label = ..p.signif..)
  labs(x = NULL, y = r_label)+
  theme_bw(base_size = 18)+
    scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  theme(legend.position = "none",
        axis.title.y = element_text(size=14),
...
seasonscomp_byyear_SuppFig <- seasonscomp_byyear_NEP / GPPseasonscomp_byyear / Rseasonscomp_byyear

# ggsave(filename = "../../Figures/SupFigX_SeasonsComp_byyear.png",
#        seasonscomp_byyear_SuppFig, device = "png", width = 350, height = 200, units = "mm")



####  Making seasonal comps across years for GPP, R, and NEP - Fig 4 c,d,e
...
left_join(boxdata_grouped, dunnR_seasons_letters_list, by = c("Season" = "Group")) %>% 
  ...
  geom_boxplot(aes(fill = Season), position = position_dodge(preserve = "single"), alpha = 0.5, outlier.alpha = 0.5)+ #, outlier.alpha = 0
  geom_jitter(aes(shape = as.character(corrected_Year)) , position=position_jitter(0.3), size = 3, fill = "black")+
  scale_shape_manual(values = c(0,1,2,5,6,7)) +
  # stat_summary(fun = mean, geom="point", shape=22, size=5, color="black", fill="black") + #point mean
  #stat_compare_means(method = "kruskal.test", label.y = 2.9, label.x = 1.3, size = 6)+
    stat_compare_means(method = "kruskal.test", label.y = 2.9, label.x = 1.3, size = 6, aes(label = sprintf("Kruskal-Wallis, p = %5.2f", as.numeric(..p.format..))))+ #round to 2 digits
  geom_text(aes(x = Season, label = Letter), y = 2.75, size = 6)+
  ylim(0,3)+
  labs(fill = element_blank(),
       shape = "Year",
       x = element_blank(),
...
NEPseasonscomp_acrossyears <- boxdata_grouped %>% 
  ...
  geom_hline(yintercept = 0, linetype=2, size = 0.75) +
  # stat_summary(fun = mean, geom="point", shape=22, size=5, color="black", fill="black") + #point mean
  #stat_compare_means(method = "kruskal.test", label.y = 0.45, label.x = 1.3, size = 6)+
      stat_compare_means(method = "kruskal.test", label.y = 0.45, label.x = 1.3, size = 6, aes(label = sprintf("Kruskal-Wallis, p = %5.2f", as.numeric(..p.format..))))+ #round to 2 digits
  ylim(-1.5, 0.5)+
  labs(fill = element_blank(),
       shape = "Year",
       x = element_blank(),
...
#combine plots
NEPseasonscomp_acrossyears + GPPseasonscomp_acrossyears + Rseasonscomp_acrossyears


```


## Fig4 seasons and within winter comp

```{r}

fig4top <- wintercomp_byyear_NEP + wintercomp_acrossyears + plot_layout(widths = c(6,2)) # + plot_annotation(tag_levels = 'A')

fig4bot<- Rseasonscomp_acrossyears + GPPseasonscomp_acrossyears + NEPseasonscomp_acrossyears

fig4 <- fig4top / fig4bot + plot_annotation(tag_levels = 'a')

ggsave(filename = "../../Figures/Fig4.png",
       fig4, device = "png", width = 350, height = 200, units = "mm")
...
## 9m DO under ice and temp profiles 

```{r}

edi_catwalk <- read_csv("../../Data/EDI2023/FCR_Catwalk_2018_2022.csv")


#### DO 
do_underice <- edi_catwalk %>% 
  mutate(Date = as.Date(DateTime),
         Month = month(Date)) %>% 
  filter(Month %in% c(12,1,2)) %>% 
  group_by(Date) %>% 
  summarise(DO_9m = mean(RDO_mgL_9_adjusted, na.rm = T),
            DO_5m = mean(RDO_mgL_5_adjusted, na.rm = T),
            DO_1.6m = mean(EXODO_mgL_1, na.rm = T)) %>% 
  pivot_longer(-1)
...
seasons_do_underice <- left_join(seasons_ts_operational, do_underice, by = "Date") %>% 
  filter(Season == "Winter")

DOprofile_underice <- seasons_do_underice %>% 
  filter(Date > ymd("2018-12-01")) %>% 
  mutate(Month = month(Date),
         Day = day(Date),
         FakeYear = ifelse(Month == 12, 2023, 2024), 
         FakeDate = ymd(paste(FakeYear, Month, Day, sep = "-"))) %>% 
  ggplot(aes(x = FakeDate, y = value, color = name))+
  geom_point()+
  labs(x = element_blank(),  y = "DO (mg/L)", color = "Depth")+
  facet_wrap(~cor_Year, ncol = 1)+
  ylim(0,15)+
  theme_bw(base_size = 16)

DOprofile_underice

ggsave(filename = "../../Figures/FigS7_DOunderIce.png",
       DOprofile_underice, device = "png", width = 200, height = 250, units = "mm")
  
#### Temp 
temp_underice <- edi_catwalk %>% 
...
seasons_temp_underice <- left_join(seasons_ts_operational, temp_underice, by = "Date") %>% 
  filter(Season == "Winter")

# seasons_temp_underice %>% 
#   filter(Date > ymd("2018-12-01")) %>% 
#   mutate(name = ifelse(name == "ThermistorTemp_C_surface", "ThermistorTemp_C_0.1", name),
#          name_A = as.numeric(gsub("ThermistorTemp_C_", '', name))) %>% 
#   mutate(Month = month(Date),
#          Day = day(Date),
#          FakeYear = ifelse(Month == 12, 2023, 2024), 
#          FakeDate = ymd(paste(FakeYear, Month, Day, sep = "-"))) %>% 
#   ggplot(aes(x = FakeDate, y = value, color = as.character(name_A)))+
#   geom_point()+
#   labs(x = element_blank(),  y = "Temp (C)", color = "Depth")+
#   facet_wrap(~cor_Year)+
#   theme_bw()+
#   ylim(2,4.5)
