...
Rseasonscomp_acrossyears <- 
  ...
  stat_summary(fun = median, shape = 4, size = 2)+
  geom_jitter(aes(shape = as.character(corrected_Year)), fill = "black", position=position_jitter(0.1), size = 3)+
...
NEPseasonscomp_acrossyears <- boxdata_grouped %>% 
  ...
  #geom_boxplot(aes(fill = Season), position = position_dodge(preserve = "single"), alpha = 0.5, outlier.shape = NA)+ 
  stat_summary(fun = median, shape = 4, size = 2)+
  geom_jitter(aes(shape = as.character(corrected_Year)), fill = "black",  #, fill = as.character(corrected_Year)
...
ggsave(filename = "../../Figures/Fig4_V3.jpg",
