...
seasons_ts_operational <- read.csv("../../Data/Seasons_operational_Ice.csv")
...
seasons_ts_winterOper_summerPhys <- read.csv("../../Data/Seasons_Ice_corrected_groupings.csv")
...
seasons_ts_phys <- read.csv("../../Data/Seasons_Ice_physical_9nov23.csv")
...
seasons_ts_solar <- read.csv("../../Data/Seasons_solar_Ice.csv")
...
fig2_all_seasons_data <- metab_seasons_winterOper_summerPhys %>% 
...
fig2_nowinter_data <- metab_seasons_winterOper_summerPhys %>% 
...
fig2_data <- left_join(fig2_all_seasons_data, fig2_nowinter_data, by = "corrected_Year")
 ...
 x <- fig2_data %>% 
  ...
  ylim(-0.4, 0.1)+
  stat_compare_means(method = "t.test", label.y = 0.1, label.x = 1.3)+ #t test 
  theme_bw()+
...
t.test(fig2_data_ttest$annual_R_nowinter, fig2_data_ttest$annual_R_Allseasons, paired = T)
t.test(fig2_data_ttest$annual_GPP_nowinter, fig2_data_ttest$annual_GPP_Allseasons, paired = T)
...
#GPP and R plots
 fig2_data_operational %>% 
  filter(corrected_Year %in% c(2016,2018,2019,2020,2021)) %>% 
  select(corrected_Year, annual_GPP_nowinter, annual_GPP_Allseasons) %>% 
  pivot_longer(-1) %>% 
  mutate(name = ifelse(name == "annual_GPP_Allseasons", "All Seasons", "No Winter")) %>% 
  ggplot(mapping = aes(x= name, y = value))+
  geom_boxplot()+
  ggtitle("GPP")+
  geom_jitter(aes(color = as.factor(corrected_Year)), size = 4)+
    stat_compare_means(method = "t.test", paired = T, label.y = 1.5, label.x = 1.3)+ #t test 
  theme_bw()

 fig2_data_operational %>% 
  filter(corrected_Year %in% c(2016,2018,2019,2020,2021)) %>% 
  select(corrected_Year, annual_R_nowinter, annual_R_Allseasons) %>% 
  pivot_longer(-1) %>% 
  mutate(name = ifelse(name == "annual_R_Allseasons", "All Seasons", "No Winter")) %>% 
  ggplot(mapping = aes(x= name, y = value))+
  geom_boxplot()+
  ggtitle("R")+
  geom_jitter(aes(color = as.factor(corrected_Year)), size = 4)+
 ...
 kruskalGPP_seasons <- kruskal.test(GPP ~ Season, data = fig2_data_operational)
...
fig2_winterVSnowinter_vsjustwinter <- fig2_data_operational %>% 
  ...
  geom_boxplot()+
  geom_jitter(aes(color = as.factor(corrected_Year)), size = 4, position=position_jitter(0))+
    stat_compare_means(method = "kruskal.test", label.y = -0.4, label.x = 1.3, size = 6)+
  labs(x = element_blank(), y = nep_label,
       color = "Year")+
  geom_hline(yintercept = 0, linetype=2, size = 0.75) +
  ylim(-0.4, 0.1)+
  ...
  theme(legend.position = "top",
        ...
        axis.line = element_line(color="black"))+
  theme(axis.title.x = element_text(size=18),  axis.title.y = element_text(size=18),  legend.title = element_text(size=14),
...
ggsave(filename = "../../Figures/MS_final/Fig2_v2.png",
...
seasons_summaryStats_clean <- metab_seasons_long %>% 
  ...
  mutate(median_sd = paste(round(median, 3), round(sd, 3), sep = " +- "),
         range =   paste(round(min, 3), round(max, 3), sep = " - ")  ) %>% 
  select(name, Season, median_sd, range)
...
iceonoff_summaryStats_clean <- metab_seasons_long %>% 
  ...
  mutate(median_sd = paste(round(median, 3), round(sd, 3), sep = " +- "),
         range =   paste(round(min, 3), round(max, 3), sep = " - ")  ) %>% 
  select(name, Ice_on_off, median_sd, range)
...
ggsave(filename = "../../Figures/MS_final/SupFig_S5.png",
...
ggsave(filename = "../../Figures/MS_final/SupFig_S6.png",
...
# ggsave(filename = "../../Figures/MS_final/IceCover_fig1b.png",
...
# ggsave(filename = "../../Figures/MS_final/SuppFig_WinterComp_byyears.png",
...
wintercomp_acrossyears <- winterNEP %>% 
  ...
  geom_hline(yintercept = 0, linetype=2, size = 0.75) +
  stat_compare_means(method = "wilcox.test", label.y = 0.4, label.x = 1, size = 6)+
  ylim(-1.5, 0.5)+
  labs(fill = element_blank(),
       shape = "Year",
       x = element_blank(),
...
# ggsave(filename = "../../Figures/MS_final/SupFig_SeasonsComp_byyear.png",
...
ggsave(filename = "../../Figures/MS_final/Fig4.png",
