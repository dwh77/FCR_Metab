...
library(tidyverse)
library(ggpubr)
metaboutput <- read.csv("../../Data/Model_Output/MetabOutput_QAQC_15_22_09jun23.csv")
head(metaboutput)

metaboutput1 <- metaboutput %>% 
  filter(R_QAQC < 18) %>% #remove three high R and NEP values that have poor model fits 
...
## Try putting all mutates in one chunk
...
metab_simulations_TEST <- Dates_FIN %>% 
  mutate(
    #1 day sims
    NEP_1day = ifelse(Date == ymd("2018-12-20"), 0.1313906830, NEP),
    NEP_1day = ifelse(Date == ymd("2020-12-20"), 0.02237530, NEP_1day),
    
    #5 day sims
    NEP_5day = ifelse(Date %in% seq(ymd("2018-12-20"), ymd("2018-12-24"), by = "1 day"), 0.1313906830, NEP),
    NEP_5day = ifelse(Date %in% seq(ymd("2020-12-20"), ymd("2020-12-24"), by = "1 day"), 0.02237530, NEP_5day),
    
    #10 day sims
    NEP_10day = ifelse(Date %in% seq(ymd("2018-12-20"), ymd("2018-12-29"), by = "1 day"), 0.1313906830, NEP),
    NEP_10day = ifelse(Date %in% seq(ymd("2020-12-31"), ymd("2021-01-09"), by = "1 day"), 0.02237530, NEP_10day),
    
    #20 day sims
    NEP_20day = ifelse(Date %in% seq(ymd("2018-12-20"), ymd("2019-01-08"), by = "1 day"), 0.1313906830, NEP),
    NEP_20day = ifelse(Date %in% icefree2021, 0.02237530, NEP_20day),
    
    #40 day sims
    NEP_40day = ifelse(Date %in% icefree2019_40days, 0.1313906830, NEP),
    NEP_40day = ifelse(Date %in% icefree2021_40days, 0.02237530, NEP_40day),

    #80 day sims
    NEP_80day = ifelse(Date %in% icefree2019_80days, 0.1313906830, NEP),
    NEP_80day = ifelse(Date %in% icefree2021_80days, 0.02237530, NEP)
