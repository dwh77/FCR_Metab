...
thermal_metrics <- read.csv("../../Data/thermalmetric_calcs.csv")

ice <- read_csv("../../Data/EDI2023/Ice_Data_2013_2022.csv")
ice <- ice %>% 
  filter(Reservoir == "FCR")

#I then manually looked through the ice and thermal metrics data frames to determine days that had ice cover (those time periods are defined in the chunk starting on line 23) and periods of thermal stratification using at 0.1 kg m^-3 thershold of difference from 1m to 8m depth (column Strat_Dens_0.1), where the start of stratification in a year was defined as the first day that the threshold was greater than 0.1 ("Strat") and remained that for at least 14 days, and stratificiation ended the first day that the threshold was less than 0.1 ("Not_Strat")
...
seasons_ts <- rbind(fall, spring, summer, winter)
seasons_ts <- dplyr::arrange(seasons_ts, Date)
```
...
##bind seasons to ice dates 

```{r}
head(binded_icedata)
head(seasons_ts)

joined_seasons_ice <- left_join(seasons_ts, binded_icedata, by = "Date")

write.csv(joined_seasons_ice, "../../Data/Seasons_Ice_corrected_groupings.csv", row.names = F)
```
...
write.csv(solar_seasonsA, "../../Data/Seasons_solar_Ice.csv", row.names = F)
...
write.csv(operational_seasonsA, "../../Data/Seasons_operational_Ice.csv", row.names = F)
...
##Make data frames to define seasons - winter based on ice not operational above 
...
write.csv(phys_seasons_ts_ice, "../../Data/Seasons_Ice_physical_9nov23.csv", row.names = F)
